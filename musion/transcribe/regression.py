...
class RegressionPostProcessor(object):
    def __init__(self, frames_per_second=100, classes_num=88, onset_threshold=0.3, 
...
def note_detection_with_onset_offset_regress(frame_output, onset_output, 
    ...
    with frame and offset outputs.pedal_detection_with_onset_offset_regress
...
def events_to_midi(note_events, pedal_events):
    ...
    for message in message_roll:
        this_ticks = int(message['time'] * ticks_per_second)
        if this_ticks >= 0:
